...
layui.define(["element","jquery"],function(exports){
	...
	Tab.prototype.tabAdd = function(_this){
		...
		}else if(_this.attr("href") != undefined){
			var title = '';
			//已打开的窗口中不存在
			if(that.hasTab(_this.text()) == -1 && _this.siblings("dl.layui-nav-child").length == 0){
				...
				element.tabAdd(tabFilter, {
			        title : title,
			        content :"<iframe src='"+_this.attr("href")+"' data-id='"+tabIdIndex+"'></frame>",
			        id : _this.id
			    })
				//当前窗口内容
				var curmenu = {
				...
				element.tabChange(tabFilter, that.getLayId(_this.text()));
	...
	$(".closePageOther").on("click",function(){
		if($("#top_tabs li").length>2 && $("#top_tabs li.layui-this span").text()!="首页"){
			var menu = JSON.parse(window.sessionStorage.getItem("menu"));
			$("#top_tabs li").each(function(){
				if($(this).attr("lay-id") != '' && !$(this).hasClass("layui-this")){
					...
					for(var i=0;i<menu.length;i++){
						if($("#top_tabs li.layui-this span").text() == menu[i].title){
		...
		}else if($("#top_tabs li.layui-this span").text()=="首页" && $("#top_tabs li").length>1){
			$("#top_tabs li").each(function(){
				if($(this).attr("lay-id") != '' && !$(this).hasClass("layui-this")){
					element.tabDelete("bodyTab",$(this).attr("lay-id")).init();
